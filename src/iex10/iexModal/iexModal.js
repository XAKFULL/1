...
function IexModal($){
    ...
    app.classPopup = 'iexmodal-popup';
    app.classClose = 'iexmodal-close';
    app.classCloseAll = 'iexmodal-close-all';
    app.classInner = 'iexmodal-inner';
    app.classInnerWrap = 'iexmodal-inner-wrap';
    app.classBtn = 'iexmodal-show';
    app.classSprite = 'iexmodal-sprite';
    app.classContent = 'iexmodal-content';
    app.classContentWrap = 'iexmodal-content-wrap';
    app.classDebugWrap = 'iexmodal-debug-wrap';
    app.classDebugCode = 'iexmodal-debug-code';

    app.classLoading = 'iexmodal-loading';
    app.classBottom = 'iexmodal-bottom';
    app.classOverflowY = 'iexmodal-overflow-y';
    app.classOverflowX = 'iexmodal-overflow-x';
     ...
     */
    app.loadTypes = [
        'selector', 'jquery', 'html', 'url', 'id',
        'selectorArray', 'jqueryArray'
    ];
     ...
     * 3) app.callbackNames (эти параметры можно передавать только через API)
     *
     */
    app.params = [
        'selector', 'jquery',
    ...
    /**
     * В мобильном IE console отсутствует и вызов его методов вызовет ошибки JS
     */
    app.log = (window.console && window.console.log) ? console.log : function(){};

    app.validateCallback = function(callback){
    ...
    /**
     * Получает CSS-класс (модификатор), соотвествующий режиму показа
     * @returns {string}
     */
    app.getModeClass = function (mode) {
        return mode==='default' ? '' : 'iexmodal-'+mode;
    };
    ...
    app.correctPositionX = function (modal) {
        const popupWidth = modal.width || app.popupWidthDefault;
        const breakpointWidth = parseInt(popupWidth) + app.popupIndentX * 2;
        const width = app.windowWidth > breakpointWidth ? popupWidth : '';
    ...
    app.correctPositionY = function (modal) {
        const overflowY = (modal.$innerWrap.outerHeight() + parseInt(modal.$inner.css('padding-top'))*2 + app.popupIndentY*2) >= app.windowHeight;
    ...
    app.getContentWrap = function(id){
        return $body.find('.'+app.classContent+'[data-iexmodal-id="' + id + '"]').first();
    };
    ...
    app.ajaxReport = function(url, jqXHR){
        ...
        let responseText = jqXHR.responseJSON ? JSON.stringify(jqXHR.responseJSON) : jqXHR.responseText;
    ...
    app.validateParams = function(rawParams){
        ...
        $.each(app.params, function(key, param) {
            if ( app.inArray(param, app.callbackNames) ) {
                validParams[param] = app.validateCallback( rawParams[param] );
            } else if ( app.inArray(param, ['jquery','jqueryArray','selectorArray']) || (typeof rawParams[param] === 'boolean' ) ) {
                validParams[param] = rawParams[param];
            } else {
                validParams[param] = $.trim(rawParams[param] || '');
            }

            if ( param==='url' && validParams[param] &&
                (
            ...
            if ( param==='overlay' && (typeof validParams[param] !== 'boolean') ) {
        ...
        $.each(app.callbackNames, function (key, name) {
            if ( !validParams[name] ) {
    ...
    app.remove = function (modal) {
        if (modal.loadType==='selector' || modal.loadType==='jquery') {
            modal.$currContent.insertAfter(modal.$contentPlaceholder);
            modal.$contentPlaceholder.remove();
        }
        if (modal.loadType==='selectorArray' || modal.loadType==='jqueryArray') {
            $.each( modal.array, function (key, item) {
     ...
     * - selectorArray {array} - массив jQuery-селекторов на разные врапперы с html-контентом
     * - jqueryArray {array} - массив jQuery-ссылк на разные врапперы враппер с html-контентом
     ...
     * - mode {string} - Режим показа модального окна: 'default', 'fullscreen', 'top', 'right', 'bottom', 'left' (по-умолчанию 'default')
     ...
     * Из списка параметров selector, jquery, selectorArray, jqueryArray, html, url, id одновременно можно передать только ОДИН!
     */
    app.show = function (params) {
        ...
        if (modal.loadType==='selector' || modal.loadType==='jquery') {
            let $currContent = modal.loadType==='selector' ? $(params['selector']) : params['jquery'];
            if ( $currContent.length ) {
                $currContent = $currContent.first();
                modal.$currContent = $currContent;
                modal.$contentPlaceholder = app.insertPlaceholder(modal.$currContent);
                modal.$currContent.appendTo( modal.$innerWrap );
                app.onAfterContentLoad();
            }
        }

        if (modal.loadType==='selectorArray' || modal.loadType==='jqueryArray') {
            $.each( params[modal.loadType], function (key, item) {
                let $currContent = modal.loadType==='selectorArray' ? $(item) : item;
                if ( $currContent.length ) {
                    $currContent = $currContent.first();
                    modal.array[key] = {
                        '$currContent': $currContent,
                        '$contentPlaceholder': app.insertPlaceholder($currContent),
                    };
                    $currContent.appendTo( modal.$innerWrap );
                }
            } );
            app.onAfterContentLoad();
        }
    ...
    /**
     * Закрыть одно текущее модальное окно (самое верхнее)
     * @return {object} app
     */
    app.close = function () {
        ...
        if (app.modalsStack.length===1) {
            $window.disablescroll('off');
            $window.off('resize orientationchange', app.windowRelatedActions);
        }
        modal.$wrap.fadeOut(app.fadeOutTime, app.onAfterFadeOut);
        return app;
    };
     ...
     */
    app.disableScrollKeys = {
        'default': [32, 33, 34, 35, 36, 37, 38, 39, 40],
        'form': [33, 34] // для корректной работы полей форм в модальном окне
    };

    /**
     * Массив подстрок текста, при наличии которых в контенте модального окна будет считаться,
     * что в окне присутсвуют поля формы, а значит не нужно блокировать клавиши, необходимые для использования этих полей
     *
     * @type {string[]}
     */
    app.disableScrollTags = [
        'input', 'textarea'
    ];

    /**
     * Активация плагина $.disablescroll.
     * Если в отображаемом модальном окне присутствуют поля формы (перечислены в app.disableScrollTags)
     * не блокируем кнопки/клавиши, которые используются на формах (по-умолчанию они блокируются)
     *
     * @param modal
     */
    app.initDisableScroll = function(modal){
        var
            keyCodes = app.disableScrollKeys['default'],
            html = modal.$innerWrap.html(),
            regex = new RegExp( app.disableScrollTags.join('|') );

        if ( regex.exec(html) ) {
            keyCodes = app.disableScrollKeys['form'];
        }

        $window.disablescroll('on', {
            scrollEventKeys : keyCodes
        });
    };

    /**
     * Системный коллбэк для возможности асинхронной (аякс) загрузки контента
     */
    app.onAfterContentLoad = function () {
        var modal = app.getModal();

        if (app.modalsStack.length===1) {
            app.initDisableScroll(modal);
            app.windowRelatedActions();
            $window.on('resize orientationchange', app.windowRelatedActions);
        }
    ...
    /**
     * Системный коллбэк, выполняемый после завершения анимации скрытия окна
     */
    app.onAfterFadeOut = function () {
        var modal = app.getModal();
        if (!modal) return app;
    ...
    /**
     * Системный коллбэк, используемый в app.show() для закрытия текущего окна,
     * чтобы показать новый контент только после завершения анимации закрытия
     */
    app.onAfterClose = function () {};
    ...
    app.closeAll = function () {
        if ( !app.modalsStack.length ) return app;

        var $wraps = $();
        $.each( app.modals, function (id, modal) {
            $wraps = $wraps.add(modal.$wrap);
        });
        $wraps.fadeOut(app.fadeOutTime, function () {
            for (var i = app.modalsStack.length; i>0; i--) {
                app.remove( app.getModal() );
            }
        });
    ...
    app.__init = function () {
        ...
        $window.on('keyup', app.handleEscape);
        ...
        $.each(app.callbackNames, function (key,val){
            app.params.push(val);
        });
    };
...
/**
 * $.disablescroll (модифицированн для iexModal)
 * Author: Josh Harrison - aloof.co
 * https://github.com/realjoshharrison/jquery-disablescroll/blob/master/jquery.disablescroll.js
 *
...
;(function($) {
    ...
    function UserScrollDisabler($container) {
    ...
    $.fn.disablescroll = function(method, options) {
        ...
        if(!instance) {
            instance = new UserScrollDisabler(this);
        }

        if(instance && method==="on") {
            instance.on(options);
        }

        else if(instance && method==="off") {
            instance.off();
        }
